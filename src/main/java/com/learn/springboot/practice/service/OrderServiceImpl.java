...
public class OrderServiceImpl implements OrderService {
    ...
    @Autowired
    private StateMachinePersister<OrderStatusEnum, OrderEventEnum, Order> persister;
    ...
    public Order pay(int id) {
        ...
        log.info("线程名称：" + Thread.currentThread().getName() + " 尝试支付，订单号：" + id);
        Message<OrderEventEnum> message = MessageBuilder.withPayload(OrderEventEnum.PAYED).setHeader("order", order).build();
    ...
    public Order deliver(int id) {
        ...
        if (!sendEvent(MessageBuilder.withPayload(OrderEventEnum.DELIVERY).setHeader("order", order).build(), orders.get(id))) {
    ...
    public Order receive(int id) {
        ...
        if (!sendEvent(MessageBuilder.withPayload(OrderEventEnum.RECEIVED).setHeader("order", order).build(), orders.get(id))) {
     ...
     * @return
     */
    private synchronized boolean sendEvent(Message<OrderEventEnum> message, Order order) {
